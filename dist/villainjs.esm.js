...
var Translator = function (_EventEmitter) {
  ...
  Translator.prototype.translate = function translate(keys) {
    ...
    if (res && typeof res !== 'string' && noObject.indexOf(resType) < 0 && !(joinArrays && resType === '[object Array]')) {
      ...
      for (var m in res) {
        copy$$1[m] = this.translate('' + key + keySeparator + m, _extends$3({ joinArrays: false, ns: namespaces }, options));
      }
      res = copy$$1;
    }
...
class BlockStore {
  ...
  del(store, id) {
    this.stores[store] = underscore.filter(this.stores[store], block => parseInt(block.id, 10) !== parseInt(id, 10));
  }
...
underscore.mixin({
    ...
    classify: function (str) {
        return underscore.titleize(String(str).replace(/[\W_]/g, ' ')).replace(/\s/g, '');
    },

    classifyList: function (a) {
        return underscore.map(a, function (i) {
            return underscore.classify(i);
        });
    },
    ...
    underscored: function (str) {
        return underscore.trim(str).replace(/([a-z\d])([A-Z]+)/g, '$1_$2').replace(/[-\s]+/g, '_').toLowerCase();
    },
    ...
    flattern: function (obj) {
        var x = {};
        underscore.each(obj, function (a, b) {
            x[underscore.isArray(obj) ? a : b] = true;
        });
        return x;
    },
...
const Plus = backbone.View.extend({
  ...
  template: underscore.template('<button class="villain-add-block-button">+</button>'),
  blockSelectionTemplate: underscore.template('<div class="villain-block-selection"><%= content %><button class="villain-close-picker">×</button></div>'),
  events: {
  ...
  initialize(options) {
    ...
    this.$el.attr('id', underscore.uniqueId('villain-plus-'));
    this.render();
  },
  ...
  onClickAddBlock(e) {
    ...
    const $addBlockButton = jquery(e.currentTarget);
...
const Block = backbone.View.extend({
  ...
  type: 'base',
  template: underscore.template('base'),
  resizeSetup: true,
  hasToolbar: false,
  store: 'main',

  wrapperTemplate: underscore.template(['<div class="villain-block-inner"><%= content %><%= actions %></div>'].join('\n')),

  actionsTemplate: underscore.template(['<div class="villain-actions">', '  <div class="villain-action-button villain-action-button-setup">', '    <i class="fa fa-cog fa-fw"></i>', '  </div>', '  <div class="villain-action-button villain-action-button-del">', '    <i class="fa fa-trash-alt fa-fw"></i>', '  </div>', '  <div class="villain-action-button villain-action-button-move" draggable="true">', '    <i class="fa fa-arrows-alt fa-fw"></i>', '  </div>', '</div>'].join('\n')),

  setupTemplate: underscore.template('<div class="villain-setup-block" />'),
  ...
  events() {
    return underscore.extend({}, this.originalEvents, this.additionalEvents);
  },
  ...
  onPaste(e) {
    ...
    if (e && e.originalEvent.clipboardData && e.originalEvent.clipboardData.getData) {
      ...
      if (jquery.isArray(clipboardTypes)) {
  ...
  hasData() {
    return this.data ? !underscore.isEmpty(this.data) : false;
  },
  ...
  refreshContentBlock() {
    ...
    this.$content.html(jquery(block).html());
...
class Markup {
    ...
    static toHTML(markdown, type) {
        // MD -> HTML
        if (underscore.isUndefined(markdown)) {
            return "";
        }

        type = underscore.classify(type);

        var html = markdown,
            shouldWrap = type === 'Text';

        if (underscore.isUndefined(shouldWrap)) {
            shouldWrap = false;
        }
        ...
        // with a slash.
        html = underscore.reverse(underscore.reverse(html).replace(/_(?!\\)((_\\|[^_])*)_(?=$|[^\\])/gm, function (match, p1) {
...
const Text = Block.extend({
  ...
  template: underscore.template('<div class="villain-text-block villain-content" data-text-type="<%= type %>"><%= content %></div>'),
  ...
  setup() {
    ...
    const radios = HTMLUtils.createRadios(this.editor.i18n.t('text:type'), `text-type-${this.dataId}`, [{ name: this.editor.i18n.t('text:paragraph'), val: 'paragraph' }, { name: this.editor.i18n.t('text:lead'), val: 'lead' }, { name: this.editor.i18n.t('text:custom'), val: 'custom' }], type, [{
      ev: 'change',
      fn: e => {
        const val = jquery(e.target).val();
        if (val === 'custom') {
          jquery(`input[name="text-custom-${this.dataId}"]`).prop('disabled', false);
        } else {
          jquery(`input[name="text-custom-${this.dataId}"]`).prop('disabled', true);
        }
    ...
    const $customField = HTMLUtils.createInput(this.editor.i18n.t('text:custom'), `text-custom-${this.dataId}`, initialCustomClass, {
      ev: 'keyup',
      fn: e => {
        this.setDataProperty('type', jquery(e.target).val());
        this.refreshContentBlock();
        this.$content.attr('data-text-type', jquery(e.target).val());
      }
    });
...
const Header = Block.extend({
  ...
  setup() {
    ...
    const headerSizeRadios = HTMLUtils.createRadios(this.editor.i18n.t('header:header_size'), `header-size-${this.dataId}`, [{ name: 'H1', val: 1 }, { name: 'H2', val: 2 }, { name: 'H3', val: 3 }, { name: 'H4', val: 4 }, { name: 'H5', val: 5 }, { name: 'H6', val: 6 }], level, [{
      ev: 'change',
      fn: e => {
        this.setDataProperty('level', parseInt(jquery(e.target).val(), 10));
        this.refreshContentBlock();
        this.$content.attr('data-header-level', jquery(e.target).val());
      }
    }]);

    const anchorInput = HTMLUtils.createInput(this.editor.i18n.t('header:anchor'), `header-anchor-${this.dataId}`, anchor, {
      ev: 'keyup',
      fn: e => {
        this.setDataProperty('anchor', jquery(e.target).val());
      }
    });
...
const Blockquote = Block.extend({
  ...
  template: underscore.template('<div class="villain-quote-block villain-content"><blockquote contenteditable="true"><%= content %></blockquote><cite contenteditable="true"><%= cite %></cite></div>'),
  ...
  getJSON: function getJSON() {
    ...
    const cite = jquery('cite', this.$content).html();
...
const List = Block.extend({
  ...
  template: underscore.template(['<div class="villain-text-block villain-text-block-list villain-content" contenteditable="true">', '  <%= content %>', '</div>'].join('\n')),
  ...
  getJSON() {
    const textNode = this.getTextBlock().html().replace(/<\/li>/mg, '\n').replace(/<\/?[^>]+(>|$)/g, '').replace(/^(.+)$/mg, ' - $1');
    return {
      type: this.type,
      data: {
        text: textNode
...
var _defaultParams2 = _interopRequireWildcard(defaultParams_1);
...
var _injectedHTML2 = _interopRequireWildcard(injectedHtml);
...
var setFocusStyle = function setFocusStyle($button, bgColor) {
  var rgbColor = utils.hexToRgb(bgColor);
...
var openModal = function openModal(callback) {
  var $modal = getModal();
  handleDom.fadeIn(getOverlay(), 10);
  handleDom.show($modal);
  handleDom.addClass($modal, 'showSweetAlert');
  handleDom.removeClass($modal, 'hideSweetAlert');
  ...
  setTimeout(function () {
    handleDom.addClass($modal, 'visible');
  }, 500);
...
var resetInput = function resetInput() {
  ...
  handleDom.removeClass($modal, 'show-input');
...
var resetInputError = function resetInputError(event) {
  ...
  handleDom.removeClass($errorIcon, 'show');

  var $errorContainer = $modal.querySelector('.sa-error-container');
  handleDom.removeClass($errorContainer, 'show');
};
...
var fixVerticalPosition = function fixVerticalPosition() {
  var $modal = getModal();
  $modal.style.marginTop = handleDom.getTopMargin(getModal());
};
...
var handleButton = function handleButton(event, params, modal) {
  ...
  var modalIsVisible = handleDom.hasClass(modal, 'visible');
  ...
  if (targetedConfirm && params.confirmButtonColor) {
    ...
    hoverColor = utils.colorLuminance(normalColor, -0.04);
    activeColor = utils.colorLuminance(normalColor, -0.14);
  }
  ...
  switch (e.type) {
    ...
    case 'click':
      ...
      var clickedOnModalChild = handleDom.isDescendant(modal, target);
      ...
      } else if (handleDom.isDescendant(modal, target) && target.tagName === 'BUTTON') {
        sweetAlert.close();
      }
      break;
  }
...
var handleConfirm = function handleConfirm(modal, params) {
  ...
  if (handleDom.hasClass(modal, 'show-input')) {
...
var handleKeyDown = function handleKeyDown(event, params, modal) {
  ...
  if (keyCode === 9) {
    ...
    handleDom.stopEventPropagation(e);
    $targetElement.focus();

    if (params.confirmButtonColor) {
      handleSwalDom.setFocusStyle($targetElement, params.confirmButtonColor);
    }
  } else {
    if (keyCode === 13) {
      ...
      handleDom.fireClick($targetElement, e);
    } else {
...
var setParameters = function setParameters(params) {
  var modal = handleSwalDom.getModal();
   ...
   * Title
   */
  $title.innerHTML = params.html ? params.title : handleDom.escapeHtml(params.title).split('\n').join('<br>');

  /*
   * Text
   */
  $text.innerHTML = params.html ? params.text : handleDom.escapeHtml(params.text || '').split('\n').join('<br>');
  if (params.text) handleDom.show($text);

  /*
   * Custom class
   */
  if (params.customClass) {
    handleDom.addClass(modal, params.customClass);
    modal.setAttribute('data-custom-class', params.customClass);
  } else {
    // Find previously set classes and remove them
    var customClass = modal.getAttribute('data-custom-class');
    handleDom.removeClass(modal, customClass);
   ...
   * Icon
   */
  handleDom.hide(modal.querySelectorAll('.sa-icon'));

  if (params.type && !utils.isIE8()) {
    var _ret = (function () {
      ...
      var $input = handleSwalDom.getInput();

      // Animate icon
      switch (params.type) {

        case 'success':
          handleDom.addClass($icon, 'animate');
          handleDom.addClass($icon.querySelector('.sa-tip'), 'animateSuccessTip');
          handleDom.addClass($icon.querySelector('.sa-long'), 'animateSuccessLong');
          break;

        case 'error':
          handleDom.addClass($icon, 'animateErrorIcon');
          handleDom.addClass($icon.querySelector('.sa-x-mark'), 'animateXMark');
          break;

        case 'warning':
          handleDom.addClass($icon, 'pulseWarning');
          handleDom.addClass($icon.querySelector('.sa-body'), 'pulseWarningIns');
          handleDom.addClass($icon.querySelector('.sa-dot'), 'pulseWarningIns');
          break;
        ...
        case 'prompt':
          ...
          handleDom.addClass(modal, 'show-input');
          setTimeout(function () {
            $input.focus();
  ...
  if (params.cancelButtonText) {
    $cancelBtn.innerHTML = handleDom.escapeHtml(params.cancelButtonText);
  }
  if (params.confirmButtonText) {
    $confirmBtn.innerHTML = handleDom.escapeHtml(params.confirmButtonText);
  }
  ...
  if (params.confirmButtonColor) {
    ...
    handleSwalDom.setFocusStyle($confirmBtn, params.confirmButtonColor);
  }
...
var _handleKeyDown2 = _interopRequireWildcard(handleKey);
...
var _setParameters2 = _interopRequireWildcard(setParams);
...
exports['default'] = sweetAlert = swal = function () {
  ...
  handleDom.addClass(document.body, 'stop-scrolling');
  handleSwalDom.resetInput();
  ...
  if (customizations === undefined) {
    utils.logStr('SweetAlert expects at least 1 attribute!');
    return false;
  }

  var params = utils.extend({}, _defaultParams2['default']);
  ...
  switch (typeof customizations) {
    ...
    case 'object':
      if (customizations.title === undefined) {
        utils.logStr('Missing "title" argument!');
        return false;
      }
    ...
    default:
      utils.logStr('Unexpected type of argument! Expected "string" or "object", got ' + typeof customizations);
      return false;

  }

  _setParameters2['default'](params);
  handleSwalDom.fixVerticalPosition();
  handleSwalDom.openModal(arguments[1]);
  ...
  var onButtonEvent = function onButtonEvent(e) {
    return handleClick.handleButton(e, params, modal);
  };
  ...
  handleSwalDom.getOverlay().onclick = onButtonEvent;
...
sweetAlert.setDefaults = swal.setDefaults = function (userParams) {
  ...
  utils.extend(_defaultParams2['default'], userParams);
};
...
sweetAlert.close = swal.close = function () {
  ...
  handleDom.fadeOut(handleSwalDom.getOverlay(), 5);
  handleDom.fadeOut(modal, 5);
  handleDom.removeClass(modal, 'showSweetAlert');
  handleDom.addClass(modal, 'hideSweetAlert');
  handleDom.removeClass(modal, 'visible');

  /*
   * Reset icon animations
   */
  var $successIcon = modal.querySelector('.sa-icon.sa-success');
  handleDom.removeClass($successIcon, 'animate');
  handleDom.removeClass($successIcon.querySelector('.sa-tip'), 'animateSuccessTip');
  handleDom.removeClass($successIcon.querySelector('.sa-long'), 'animateSuccessLong');

  var $errorIcon = modal.querySelector('.sa-icon.sa-error');
  handleDom.removeClass($errorIcon, 'animateErrorIcon');
  handleDom.removeClass($errorIcon.querySelector('.sa-x-mark'), 'animateXMark');

  var $warningIcon = modal.querySelector('.sa-icon.sa-warning');
  handleDom.removeClass($warningIcon, 'pulseWarning');
  handleDom.removeClass($warningIcon.querySelector('.sa-body'), 'pulseWarningIns');
  handleDom.removeClass($warningIcon.querySelector('.sa-dot'), 'pulseWarningIns');
  ...
  handleDom.removeClass(document.body, 'stop-scrolling');
...
sweetAlert.showInputError = swal.showInputError = function (errorMessage) {
  ...
  var $errorIcon = modal.querySelector('.sa-input-error');
  handleDom.addClass($errorIcon, 'show');

  var $errorContainer = modal.querySelector('.sa-error-container');
  handleDom.addClass($errorContainer, 'show');
...
if (typeof window !== 'undefined') {
  ...
  utils.logStr('SweetAlert is a frontend module!');
}
module.exports = exports['default'];
});
...
const Image = Block.extend({
  ...
  blockName: 'image',
  blockIcon: 'fa-image',
  template: underscore.template('<div class="villain-image-block villain-content"><img class="img-responsive" src="<%= url %>" /></div>'),
  ...
  onUploadClickAfterDrop(e) {
    ...
    jquery.ajax({
      ...
      xhr() {
        const customXhr = jquery.ajaxSettings.xhr();
    ...
    }).done(jquery.proxy(function uploadCallback(retData) {
      /**
      ...
      if (retData.status === 200) {
        ...
        if ({}.hasOwnProperty.call(retData, 'image')) {
          ...
          const $image = jquery(`<img src="${imageData.src}" />`);
        ...
        if ({}.hasOwnProperty.call(retData, 'form')) {
          let inputsHtml = '';
          const inputTemplate = underscore.template(['<label><%= label %></label>', '<input type="<%= type %>" ', '       value="<%= value %>" ', '       name="<%= name %>"', '/>'].join('\n'));
          ...
          const formTemplate = underscore.template(['<form method="<%= method %>" ', '      action="<%= action %>" ', '      class="villain-form" ', '      name="<%= name %>"', '>', '<%= inputs %>', '</form>'].join('\n'));
          ...
          const $submitButton = jquery(`<input type="submit" name="${retData.form.name}-submit" value="OK" />`);
  ...
  onDropImage(e) {
    ...
    if (/image/.test(file.type)) {
      ...
      this.$('.villain-image-dropper').html(jquery('<img>', {
  ...
  setup() {
    ...
    if (!this.hasData()) {
      ...
      $imageDropper.find('.villain-image-browse-button').on('click', jquery.proxy(this.onImageBrowseButton, this));
      ...
      const data = this.getData();
      const $form = jquery(`<form name="image-meta-${this.dataId}">`);

      const titleInput = HTMLUtils.createInput(this.editor.i18n.t('images:title'), 'title', data.title, {
        ev: 'keyup',
        fn: underscore.debounce(e => {
          this.setDataProperty('title', jquery(e.target).val());
        })
      });
      const creditsInput = HTMLUtils.createInput(this.editor.i18n.t('images:credits'), 'credits', data.credits, {
        ev: 'keyup',
        fn: underscore.debounce(e => {
          this.setDataProperty('credits', jquery(e.target).val());
        })
      });
      const URLInput = HTMLUtils.createInput(this.editor.i18n.t('images:url'), 'link', data.link, {
        ev: 'keyup',
        fn: underscore.debounce(e => {
          this.setDataProperty('link', jquery(e.target).val());
        })
      });
      ...
      const $radios = HTMLUtils.createRadios(this.editor.i18n.t('images:size'), 'imagesize', sizes, data.url, [{
        ev: 'change',
        fn: e => {
          this.setUrl(jquery(e.target).val());
        }
      }]);
  ...
  onImageBrowseButton(e) {
    ...
    jquery.ajax({
      ...
      const $images = jquery('<div />');

      for (let i = 0; i < data.images.length; i += 1) {
        const img = data.images[i];
        const $storeImg = jquery(`<img src="${img.thumb}" />`);
        $storeImg.data('sizes', img.sizes).data('large', img.src).data('title', img.title).data('credits', img.credits);
        $images.append($storeImg);
      }
      $images.on('click', 'img', jquery.proxy(function setDataOnClick(ev) {
        this.setData({
          url: jquery(ev.target).data('large'),
          title: jquery(ev.target).data('title'),
          credits: jquery(ev.target).data('credits'),
          sizes: jquery(ev.target).data('sizes')
        });
        this.getData();
        this.refreshContentBlock();
        this.hideSetup();
...
const Slideshow = Block.extend({
  ...
  blockName: 'slides',
  blockIcon: 'fa-th',
  template: underscore.template(['<div class="villain-slideshow-block villain-content" contenteditable="false">', '  <h4>Slideshow</h4>', '  <%= content %>', '</div>'].join('\n')),
  ...
  renderDataImages() {
    const data = this.getData();
    if (underscore.isUndefined(data.images)) {
      return '';
    }
  ...
  getAllImageseries() {
    ...
    jquery.ajax({
      ...
      if (data.status === '200') {
        ...
        if (!underscore.isUndefined(self.data.imageseries)) {
    ...
    jquery.ajax({
      ...
      if (data.status === '200') {
        ...
        if (self.$setup.find('.imageserie-size-select').length > 0) {
          ...
          // add size dropdown
          const sizeSelect = '<label for="imageserie-size">Str:</label>' + '<select class="imageserie-size-select" ' + '        name="imageserie-size"></select>';
  ...
  setup() {
    ...
    if (!this.hasData()) {
      ...
      $select.on('change', function imageSeriesSelectChangeCallback() {
        self.getImageseries(jquery(this).val());
      });
...
const Video = Block.extend({
  ...
  template: underscore.template('<div class="villain-video-block villain-content"><%= content %></div>'),
  ...
  buildString(videoUrl) {
    ...
    underscore.each(this.providers, (provider, index) => {
      match = provider.regex.exec(videoUrl);

      if (match !== null && !underscore.isUndefined(match[1])) {
        data = {
          source: index,
          remote_id: match[1]
        };
  ...
  renderEditorHtml() {
    ...
    const embedString = this.providers[this.data.source].html.replace('{{protocol}}', window.location.protocol).replace('{{remote_id}}', this.data.remote_id).replace('{{width}}', '100%'); // for videos that can't resize automatically like vine
...
const Gmap = Block.extend({
  ...
  template: underscore.template('<div class="villain-map-block villain-content"><%= content %></div>'),
  ...
  buildString(mapUrl) {
    ...
    underscore.each(this.providers, function loopProviders(provider, index) {
  ...
  renderEditorHtml() {
    ...
    const embedString = this.providers[this.data.source].html.replace('{{protocol}}', 'https:').replace('{{embed_url}}', this.data.embed_url).replace('{{width}}', '100%');
...
const Divider = Block.extend({
  ...
  blockName: 'hr',
  blockIcon: 'fa-minus',
  template: underscore.template('<div class="villain-divider-block villain-content"><hr></div>'),
...
const Html = Block.extend({
  ...
  blockName: 'html',
  blockIcon: 'fa-code',
  template: underscore.template('<div class="villain-html-block villain-content"><textarea><%= content %></textarea></div>'),
...
const Markdown = Block.extend({
  ...
  template: underscore.template('<div class="villain-md-block villain-content"><textarea><%= content %></textarea></div>'),
...
const Columns = Block.extend({
  ...
  template: underscore.template('<div id="villain-column-row-<%= columnId %>" class="row"></div>'),
  columnTemplate: underscore.template('<div class="<%= columnClass %>"></div>'),
  ...
  createColumns($row, data, store) {
    for (let i = 0; i <= data.length - 1; i += 1) {
      ...
      let $column = jquery(`<div class="${columnClass}"></div>`);
  ...
  parseBlocks(blocks) {
    ...
    jquery(blocks).children('.villain-block-wrapper').each(function parseAndPushBlocks() {
  ...
  parseBlock(block) {
    if (jquery(block).attr('data-block-type') === 'columns') {
      const id = jquery(block).attr('data-block-id');
      const subCols = jquery(`#villain-column-row-${id}`, jquery(block)).children();
      return this.parseColumns(subCols);
    }

    const blockStore = jquery(block).attr('data-blockstore');
    const blockId = jquery(block).attr('data-block-id');
  ...
  updateColumnCount(e) {
    const self = this;
    const columnCount = jquery(e.target).val();
    this.$('.villain-column-widths').remove();
    const columnCountWrapper = jquery('<div class="villain-column-widths" />');
...
class BlockRegistry {
  ...
  checkBlocks() {
    for (let i = 0; i < this.registry.length; i += 1) {
      ...
      if (underscore.isUndefined(b.cls)) {
        throw new VillainError(`Villain: Missing block source for ${b.name}! Please ensure it is included.`);
      }
    }
  }

  getBlockClassByType(type) {
    const b = underscore.find(this.registry, { name: underscore(type).capitalize() });
    if (b) {
      return b.cls;
    }
...
const Editor = backbone.View.extend({
  ...
  initialize(options) {
    this.instanceId = uid_1();
    this.eventBus = underscore.extend({}, backbone.Events);
    // create a blockstore
    ...
    this.$textArea = jquery(options.textArea) || this.textArea;
    ...
    $sourceView.on('click', 'span', jquery.proxy(this.onToggleMask, this));
    $sourceView.insertBefore(this.$textArea);

    this.eventBus.on('source:toggle', this.toggleSource, this);

    const $v = jquery(`<div class="villain-editor" data-villain-instance="${this.instanceId}"></div>`);
  ...
  setOptions(options) {
    if (underscore.isUndefined(options.imageSeries) || underscore.isUndefined(options.baseURL)) {
      throw new VillainError('Villain: baseURL and imageSeries MUST be set on initialization.');
    }

    const newOpts = underscore.extend(options, {
  ...
  render() {
    ...
    for (let i = 0; i <= this.data.length - 1; i += 1) {
      ...
      if (BlockClass !== false) {
        ...
        throw new VillainError(`Villain: No class found for type: ${blockJson.type}`);
      }
    }
    return this;
  },
  ...
  onToggleMask() {
    jquery(`.villain-toggle-source[data-editor="${this.instanceId}"] .villain-toggle-menu`).toggle();
  ...
  organizeMode() {
    jquery('.villain-block-wrapper').toggleClass('organize');
    jquery('.villain-block-wrapper[data-block-type="columns"]').removeClass('organize');
    jquery('.organize .villain-content').hide();
  },
  ...
  getJSON() {
    ...
    this.$('.villain-block-wrapper').each(function pushEachBlockWrapper() {
      ...
      const block = self.blockStore.getBlockById('main', jquery(this).data('block-id'));
      if (block !== false) {
  ...
  onDragEnterDroppable(e) {
    jquery('.villain-add-block-button', e.currentTarget).addClass('drop-hover');
    e.preventDefault();
    e.stopPropagation();
  },

  onDragLeaveDroppable(e) {
    jquery('.villain-add-block-button', e.currentTarget).removeClass('drop-hover');
    e.preventDefault();
    e.stopPropagation();
  },
  ...
  onDropDroppable(e) {
    const target = e.currentTarget;
    if (jquery(target).hasClass('villain-droppable') !== true) {
      e.preventDefault();
      e.stopPropagation();
      return false;
    }
    ...
    const $source = jquery(`[data-block-id=${sourceId}]`);
    const $sourceAdd = $source.next();

    jquery('.villain-add-block-button', target).removeClass('drop-hover');
    ...
    $source.insertAfter(jquery(target));
    const oldBlockStore = $source.attr('data-blockstore');
    const newBlockStore = jquery(target).attr('data-blockstore');
  ...
  onDropTextblock(e) {
    jquery(e.currentTarget).closest('.villain-block-wrapper').shake();
    e.preventDefault();
    e.stopPropagation();
    return false;
  },
  ...
  processPaste(pastedFrag) {
    ...
    if (pastedFrag.match(/(class="?Mso|style="[^"]*\bmso\-|w:WordDocument)/gi)) {
      ...
      cleanHtml = this.clean(jquery('<div>').append(cleanHtml).html(), false, true);
  ...
  clean(html) {
    ...
    const aaReg = new RegExp(`( (?!(?:${allowedAttrs.join('|')}))[a-zA-Z0-9-_]+)=((?:.(?!\\s+(?:\\S+)=|[>]|(\\/>)))+.)`, 'gi');

    cleanedHtml = cleanedHtml.replace(aaReg, '');

    // Clean style.
    const styleReg = new RegExp('style=("[a-zA-Z0-9:;\\.\\s\\(\\)\\-\\,!\\/\'%]*"|' + '\'[a-zA-Z0-9:;\\.\\s\\(\\)\\-\\,!\\/"%]*\')', 'gi');
    cleanedHtml = cleanedHtml.replace(styleReg, '');

    // Remove the class.
    const $div = jquery('<div>').append(cleanedHtml);
    $div.find('[class]:not([class^="fr-"])').each((index, el) => {
  ...
  plainPasteClean(html) {
    const $div = jquery('<div>').html(html);

    $div.find('h1, h2, h3, h4, h5, h6, pre, blockquote').each((i, el) => {
      jquery(el).replaceWith(`<p>${jquery(el).html()}</p>`);
    });

    const replacePlain = function replacePlain(i, el) {
      jquery(el).replaceWith(jquery(el).html());
    };
  ...
  removeEmptyTags(html) {
    ...
    $div.find('> div').each((i, div) => {
      jquery(div).replaceWith(`${jquery(div).html()}<br />`);
    });
...
//# sourceMappingURL=villainjs.esm.js.map
